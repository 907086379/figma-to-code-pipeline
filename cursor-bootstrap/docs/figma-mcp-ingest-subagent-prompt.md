...
2. 三段回包经 **`npm run fc:mcp:ingest:quiet`**（或 `pnpm` / `node scripts/workflow/mcp-raw-ingest.cjs ... --quiet`）落盘；按需 **`{{ENRICH}}`**。
3. 确认进程退出码为 **0**；任意失败时终端均会给出 **`log=` / `json=`**（`preflight` 与 `gate` 两类，见 `mcp-ingest-failure.json` 的 `failureKind`），仅按路径本地排查（勿把全文贴回父会话）。
